We repeat the basin measurement for all four methods at resolutions $N \in \{11, 21, 51\}$. Basin fraction is stable by $N=21$ and the method ranking is invariant, confirming that reported results do not depend on grid resolution.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.6\linewidth]{phase_h_resolution}
  \caption{Basin fraction vs.\ grid resolution. All four methods stabilise by $N=21$; ranking is preserved at all resolutions.}
  \label{fig:phase-h}
\end{figure}

\begin{table}[h]
  \centering
\begin{tabular}{lcccc}
  \toprule
  $N$ & Peer only & Meta-MAPG & Meta-PG & PG \\
  \midrule
  11 & 41.3\% & 42.1\% & 28.9\% & 29.8\% \\
  21 & 42.6\% & 42.4\% & 26.1\% & 26.3\% \\
  51 & 42.6\% & 42.0\% & 25.6\% & 26.3\% \\
  \bottomrule
\end{tabular}
  \caption{Basin fraction vs.\ grid resolution.}
  \label{tab:phase-h}
\end{table}
